...
class TradingSystemFacadeTestOld {
...
//        }
        ...
        void purchaseShoppingCartTest() {
            ...
            PaymentDetailsDto paymentDetailsDto = modelMapper.map(paymentDetails, PaymentDetailsDto.class);
            BillingAddressDto billingAddressDto = modelMapper.map(billingAddress, BillingAddressDto.class);
            Set<Receipt> receiptsExpected = setUpReceipts();
            //mock
            when(tradingSystem.purchaseShoppingCartGuest(any(), any(), any())).thenReturn(receiptsExpected.stream().collect(Collectors.toList()));
            ...
            AssertionHelperTest.assertReceipts(receiptsExpected.stream().collect(Collectors.toList()), receiptDtoActual);
        }
    }
